...
`yarn start`
Then head to `http://owkin.substrabac:3000/` `owkin.substrabac` is important for working with same site cookie policy
...
Then head to `https://owkin.substrabac:3000/`
:warning: Be sure to use `owkin.substrabac` and not `localhost` or `127.0.0.1` for being able to work with cookies.
